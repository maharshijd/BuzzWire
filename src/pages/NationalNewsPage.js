...
const NationalNewsPage = () => {
    ...
    useEffect(() => {
        const fetchNews = async () => {
            setLoading(true);
            try {
                const response = await fetch("https://newsdata.io/api/1/latest?apikey=pub_8b630121f57443c093e4762fd3d6b3e6&country=in&language=en");
                const data = await response.json();
                if (data.status === "success" && Array.isArray(data.results)) {
                    setArticles(data.results);
                } else {
                    setError("Could not fetch national news.");
                }
            } catch (err) {
    ...
    return (
        <div className="bg-gray-100">
            <PageHeader 
                title="National News" 
                subtitle="Top Stories From Across India" 
