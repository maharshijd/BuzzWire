...
// --- International News Page Component ---
...
const InternationalNewsPage = () => {
     ...
     useEffect(() => {
         const fetchNews = async () => {
             setLoading(true);
             try {
                 const response = await fetch("https://newsdata.io/api/1/latest?apikey=pub_8b630121f57443c093e4762fd3d6b3e6&language=en");
                 const data = await response.json();
                 if (data.status === "success" && Array.isArray(data.results)) {
                     setArticles(data.results);
                 } else {
                     setError("Could not fetch international news.");
                 }
             } catch (err) {
     ...
     return (
         <div className="bg-gray-100">
             <PageHeader title="International News" subtitle="Your World in a Glance" imageUrl="https://images.unsplash.com/photo-1451187580459-43490279c0fa?q=80&w=2072&auto-format&fit=crop" />
